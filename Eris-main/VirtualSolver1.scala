...
""")
    ...
    wr.write(s"A = sparse.vstack([sparse.csc_matrix(sparse.coo_matrix((get(data,0),(get(data,1),get(data,2))),shape=($m,$n))), sparse.eye($n)], format='csc')\n")
//    if(flag_null){
      wr.write(s"l = np.hstack([b, np.zeros($n)])\n")
//    }
//    else
//      {
...
//      }
    wr.write(s"u = np.hstack([b,np.inf*np.ones($n)])\n")
    wr.write(raw"""
prob = osqp.OSQP()
prob.setup(None, q, A, l, u, verbose=False)
res = prob.solve()
